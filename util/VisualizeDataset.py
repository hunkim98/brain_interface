...
class VisualizeDataset:
    ...
    def plot_dataset(
        self, data_table, columns, match, display, algo="x", filename="test"
    ):
        ...
        if len(columns) > 1:
            f, xar = plt.subplots(
                len(columns), sharex=True, sharey=False, figsize=(25, 25)
            )
        else:
            f, xar = plt.subplots()
            xar = [xar]
    ...
    def plot_xy(
        ...
        for i, (x_line, y_line) in enumerate(zip(x, y)):
            ...
            (
                plot_method(x_line, y_line, line_styles[i])
                if line_styles is not None
                else plt.plot(x_line, y_line)
            )
    ...
    def plot_binary_outliers(
        self, data_table, col, outlier_col, algo="test", filename="test"
    ):
    ...
    def plot_performances(
        ...
        for i in range(0, len(feature_subset_names)):
            ...
            for j in range(0, len(algs)):
                means.append(
                    scores_over_all_algs[i][j][2]
                )  # only plot validation results
